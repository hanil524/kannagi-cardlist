...
const SAKKA_READINGS = {
  'トレカマーケットオンリーワン': 'とれかまーけっとおんりーわん',
  '超カードゲーム市場': 'ちょうかーどげーむいちば',
  'KAMNA': 'かむな',
  'ハニトラ梅木': 'はにとらうめき',
  'はるち': 'はるち',
  'はん': 'はん',
...
const updateModalControls = (cardName, controls) => {
  ...
  countDisplay.textContent = `${currentCount}/${displayMax}`;
...
const deckBuilder = {
  ...
  _showLimitMsg(text) {
    ...
    ['touchstart', 'touchmove', 'touchend'].forEach(type => {
      message.addEventListener(type, (e) => {
        e.stopPropagation();
        e.preventDefault();
      }, { passive: false });
    });
    ...
    // シェイクエフェクト: モーダル表示中は＋ボタン、一覧ではカード要素
    const modal = document.getElementById('image-modal');
    if (modal && modal.style.display === 'flex') {
      triggerShake(document.getElementById('add-card'));
    } else if (_lastShakeTarget) {
      triggerShake(_lastShakeTarget);
    }
...
`);
  ...
  showMessage(message) {
    ...
    ['touchstart', 'touchmove', 'touchend'].forEach(type => {
      messageDiv.addEventListener(type, (e) => {
        e.stopPropagation();
        e.preventDefault();
      }, { passive: false });
    });
...
function setupModalButtonListeners(controls) {
  ...
  if (addButton) {
    addButton.onclick = (e) => {
      e.stopPropagation();
      if (currentModalCard && currentModalCardName) {
        handleModalAddCard();
      }
    };
  }
